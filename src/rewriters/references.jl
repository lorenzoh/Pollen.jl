...
function createsources!(pkgdoc::PackageDocumentation)
    ...
    df = innerjoin(pkgdoc.info[:sourcefiles],
    ...
    sourcedocs = Dict{String, Node}(name => createsourcefiledoc(path, name)
                                    for (name, path) in zip(docnames, docpaths))

    # Documents for symbols
    symboldocs = Dict{String, Node}()
    df_docstrings = outerjoin(pkgdoc.info[:symbols][:,
                                                    [:symbol_id, :name, :kind, :instance]],
                              pkgdoc.info[:docstrings],
                              on = :symbol_id)

    for row in eachrow(df_docstrings)
        children = try
            ismissing(row.docstring) ? XTree[] :
            XTree[parse(row.docstring, MarkdownFormat())]
        catch e
        ...
        if row.kind == "const"
            push!(children,
                  createcodecell(Node(:codeblock, Leaf(string(row.symbol_id)),
                                      lang = "julia"),
                                 "",
                                 row.instance))
        end
...
function rewritedoc(pkgdoc::PackageDocumentation, path, doc)
    ...
    if matches(SelectTag(:documentation), doc)
        doc = withattributes(doc,
                             merge(attrs, referencedata(attrs[:symbol_id], pkgdoc.info)))
    end
...
function referencedata(symbol_id, info)
    ...
    return merge(Dict([(k, v) for (k, v) in pairs(symbol) if k != :instance]),
...
function referencedata(symbol, info, ::Val{:module})
    ...
    symbols = [Dict(:symbol_id => row.symbol_id,
                    :name => row.name,
                    :public => row.public,
                    :kind => row.kind)
    ...
    filedocs = [joinpath(["sourcefiles", row.name, row.file])
                for row in eachrow(joinedfiles)]
...
function _getmethods(info, symbol_id)
    ...
    return [Dict(:line => row.line,
                 :file => row.file,
                 :filedoc => __getfiledoc(row),
                 :method_id => row.method_id,
                 :symbol_id => row.symbol_id,
                 :signature => row.signature) for row in eachrow(methods)]
end
...
@testset "PackageDocumentation [rewriter]" begin rewriter = PackageDocumentation([Pollen]) end
